...
import { Folder, FolderPlus, ChevronRight, ChevronDown, Edit2, MoreVertical, Trash2 } from "lucide-react"
...
interface FlashcardsFolderTreeProps {
  selectedFolderId: string | null
  onSelectFolder: (folderId: string | null) => void
}

export function FlashcardsFolderTree({ selectedFolderId, onSelectFolder }: FlashcardsFolderTreeProps) {
  ...
  const handleDeleteFolder = async (folderId: string) => {
    if (!confirm("Are you sure you want to delete this folder? Flash cards will be moved to root.")) {
      return
    }
    ...
    return (
      <div key={folder.id}>
        ...
        >
          ...
          >
            {folder.name}
            {folder._count && folder._count.flashCards > 0 && (
              <span className="ml-2 text-xs text-muted-foreground">
                ({folder._count.flashCards})
              </span>
            )}
          </button>
          <DropdownMenu>
  ...
  return (
    <>
      <div className="mb-4 flex items-center justify-between">
        <h2 className="text-lg font-semibold">Flash Card Folders</h2>
        <Button
          variant="ghost"
          size="icon"
          className="h-8 w-8"
          ...
          <DialogHeader>
            ...
            <DialogDescription>
              {parentFolderId
                ...
                : "Create a new folder for your flash cards"}
            </DialogDescription>
          </DialogHeader>
